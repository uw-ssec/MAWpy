...
def func(args):
    name, userGps, userCell, duration_constraint, spatial_constraint_gps, partitionThreshold, outputFile = args

    user = combineGPSandPhoneStops((userGps, userCell, duration_constraint, spatial_constraint_gps, partitionThreshold))
...
if __name__ == '__main__':
    ...
    while (len(usernamechunks)):
        ...
        # pool 
        tasks = [pool.apply_async(func, (task,)) for task in [(name, UserList1[name], UserList2[name], duration_constraint, spatial_constraint_gps, partitionThreshold, outputFile) for name in UserList1]]
