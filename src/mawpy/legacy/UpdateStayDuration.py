...
def update_duration(user, dur_constr, order_of_execution=1):
    ...
    for d in user.keys():
        ...
        while i < len(user[d]):  # number of traces for a given user
            if j >= len(
                ...
                dur = str(
                    int(user[d][j - 1][0])
                    + max(0, int(user[d][j - 1][9]))
                    - int(user[d][i][0])
                )  # 0 corresponds to unix_start_t and 9 the stay_dur
                for k in range(i, j, 1):
                    user[d][k][9] = (
                        dur  # update all rows between i and j for the user with the dur calculated above i.e. all values b/w i and j have same stay
                    )
            ...
            if (
                ...
                and j < len(user[d])
            ):  # [6] is stay_lat and [7] is stay_long
                ...
                for k in range(i, j, 1):
                    user[d][k][9] = (
                        dur  # update all rows between i and j for the user with the dur calculated above
                    )
                i = j
    ...
    for d in user.keys():
        for trace in user[d]:
            ...
            if float(trace[6]) == -1:
                trace[9] = -1  # if stay_lat is -1 make stay_dur as -1
            ...
            if (
                ...
                trace[6], trace[7], trace[8], trace[9] = (
                    ...
                    -1,
                    -1,
                )  # for no stay, do not give center
