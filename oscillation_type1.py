...
def oscillation_h1_oscill(user, dur_constr):
    ...
    for d in sorted(user.keys()): # need to understand how this data is fed in the function
        user[d].sort(key=lambda x: int(x[0])) # sorting takes place based on x[0] which is the date time
        for trace in user[d]: #iterating through all rows for the dictionary key (need to confirm if it is user or datetime?)
            dur_i = 1 if int(trace[9]) == -1 else int(trace[9]) # trace[9] means the column stay_dur, which tells if it is a stay point or not; -1 signifies no stay point so a duration of 1 second is assigned here
            lat = trace[3] if float(trace[6]) == -1 else trace[6] # trace[6] is stay_lat and lat is assigned the same as actual lat i.e trace[3] if stay_lat is -1 i.e. a latitude corresponding to a stay point
            long = trace[4] if float(trace[7]) == -1 else trace[7] # trace[7] is stay_long and long is assigned the same as actual long i.e trace[4] if stay_long is -1 i.e. a longitude corresponding to a stay point
            rad = trace[5] if int(trace[8]) == -1 else trace[8] # trace[8] is stay_unc and rad is assigned the same as actual uncertainity i.e trace[5] if stay_unc is -1
            stay_not = 0 if float(trace[6]) == -1 else 1 # trace[6] is stay_lat and if it is -1 the stay_not = 0 which means it is not a stay point
            tracelist.append([trace[1], trace[0], dur_i, lat, long, rad, stay_not]) # all above parameters are added in a nested list tracelist
    ...
    while i < len(tracelist) - 1:
        orig_index.append(i)
        if tracelist[i + 1][2:5] == tracelist[i][2:5]: # in case for the entries in tracelist nested list, if the GPS coordinates of two consequent lists are same, then delete one
            del tracelist[i + 1]
        else:
            i += 1
    orig_index.append(i) # only keep the index where the next trace is not at the same location
    ...
    gpslist = [(trace[3], trace[4]) for trace in tracelist] # get GPS coordinates in terms of latitude and longitude for each entry in tracelist
    # unique gps list
    uniqList = list(set(gpslist)) # take a set to get unique values
    gps_dur_count = {item: 0 for item in uniqList} # create a dictionary where the key is the unique GPS coordinates i.e (lat, long) -> the value will be the duration stayed at that location
    for tr in tracelist:
        if int(tr[2]) == 0: # if value of dur_i from tracelist is 0 then we add 1 second to the lat-long pair (Need to confirm if it will be 0?)
            gps_dur_count[(tr[3],tr[4])] += 1
        else:
            gps_dur_count[(tr[3],tr[4])] += int(tr[2]) # else we add the actual duration
    ...
    while t_start < len(tracelist):
        flag_find_circle = False # initially set up to False to check breaking of loop down
        suspSequence = []
        ...
        for t in range(t_start + 1, len(tracelist)):  # get the suspicious sequence
            if int(tracelist[t][1]) <= int(tracelist[t_start][1]) + int(tracelist[t_start][2]) + TimeWindow: # if for a given index from tracelist this condition meets
                suspSequence.append(t) # add the index of the trace from tracelist (each index corresponds to one nested list in tracelist)
                # loc1 = (float(tracelist[t][3]),float(tracelist[t][4]))
                # loc2 = (float(tracelist[t_start][3]),float(tracelist[t_start][4]))
                if tracelist[t][3:5] == tracelist[t_start][3:5]: #geopy.distance.geodesic(loc1, loc2).meters < 300:   # if the GPS coordinates match then break the loop where time condition meets from the above
        ...
        # check circles
        if flag_find_circle == True and len(suspSequence) > 2:  # not itself ## it checks if flag is true and if the list susSequence has more than 2 index
            oscillation_pairs.append(suspSequence) # append the index to oscialltion pairs as a list i.e. oscialltion pair will also be a nested list structure
            t_start = suspSequence[-1]  # + 1 # start t_start from the last index in suspSequence
        else:
    ...
    for pair in oscillation_pairs: # go through each nested list captured in oscillation pairs
        stay_indicators = [tracelist[x][-1] for x in pair] # for a given nested list, it will go to each index in this nested list, retreive the element at that index and get the saty_not variable
        if 1 in stay_indicators: # if one of the indices correspond to a stay point
            replacing = sorted(pair, key=lambda x: gps_dur_count[(tracelist[x][3],tracelist[x][4])])[-1] # for the highest numbered index, get the GPS coordinates
        else:
            replacing = pair[0] # take first index in pair
        for to_be_replaced in pair:
            replacement[to_be_replaced] = replacing # replace each index in oscialltion pair with the to_be_replaced value from above
    ...
    for d in sorted(user.keys()):
        for trace in user[d]:
            if orig_index[ind] in replacement:
                candidate_trace = tracelist[replacement[orig_index[ind]]] # get the replacement trace
                if candidate_trace[-1] == 1: # if stay point
                    trace[6], trace[7] = candidate_trace[3], candidate_trace[4] # update columns stay_lat, stay_long
                    trace[3], trace[4] = candidate_trace[3], candidate_trace[4]
                else:
                    trace[3], trace[4] = candidate_trace[3], candidate_trace[4] # else update the origninal lat and long
            ind += 1
