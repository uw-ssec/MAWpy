...
def diameterExceedCnstr(traj, i, j, spat_constr):
    ...
    if distance(traj[i][3],traj[i][4],traj[j][3],traj[j][4])>spat_constr: # check the first and last trace
        ...
        pairloc = list(combinations(loc, 2)) ### get pairwise combinations from loc -> It looks like a nested tuple [((a,b),(c,d)),((a,b),(e,f)),...]
        max_i = 0
        max_d = 0
        ...
        if distance(pairloc[max_i][0][0], pairloc[max_i][0][1], pairloc[max_i][1][0],
            ...
            while m < len(pairloc):
                if abs(pairloc[m][0][0] - pairloc[m][1][0]) < max_ln_lat[0] \
                    ...
                    del pairloc[m] ### delete those pairs having distance lesser then max_ln_lat???
                else:
...
def cluster_traceSegmentation(user, spat_constr, dur_constr):
    for day in user.keys():
        ...
        while (i<len(traj)-1): 
            ...
            if flag:
                break
            if diameterExceedCnstr(traj,i,j,spat_constr): # if spatial constraint is exceeded between i and j, then move to next segment i
                i += 1
            else: # in case spatial constraint was not broken
                j_prime = j
                gps_set = set([(round(float(traj[m][3]),5),round(float(traj[m][4]),5)) for m in range(i,j+1)]) # get umiquere gps locations
                for k in range(j_prime+1, len(traj),1): # #j: max k subject to Diameter(R,i,k)<=spat_constraint
                    if (round(float(traj[k][3]), 5), round(float(traj[k][4]), 5)) in gps_set: # if this point further than j is still in older set
                        j = k
                ...
                for k in range(i, j + 1):  # give cluster center
                    traj[k][6], traj[k][7], traj[k][9] = mean_lat, mean_long, dur # update trace values [6] is stay_lat, [7] is stay_long and [9] is stay_dur
                i = j+1 # next segment
        user[day] = traj
